...
static unsigned int joyLeft = 0, joyRight = 0;    
static uint8_t g_blur_level = 0;
...
void read_digital_pad(void)
{
	if (g_digital.connected == 1)
	{
		...
		if (joyRight)
		{
            if(g_blur_level < 0x1F) g_blur_level++;
            MEMORY_WRITE(16, VDP2(CCRNB), g_blur_level & 0x1F);
		}
		else if (joyLeft)
		{
           if(g_blur_level > 0x0) g_blur_level--;
            MEMORY_WRITE(16, VDP2(CCRNB), g_blur_level & 0x1F);
		}
...
{           
	...
	// Enable gradiation on NG2 and enable color fonction on NGB2
    MEMORY_WRITE(16, VDP2(CCCTL), (1 << 15) | (1 << 14) | (1 << 12) | 0x4);    
